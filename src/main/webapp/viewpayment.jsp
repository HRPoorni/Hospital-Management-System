    ...
    <section class="container wrapper">
      <h2 class="display-4 pt-3">Payments</h2>
          ...
          <form action="DeleteUserServlet" method="POST" >
           ...
           <td><%= cvc %></td>
           <td><%= exp_date %></td>
           <td><a href="viewpayment.jsp?id=<%= id%>">Delete</a>|<a href="updatepayment.jsp?idu=<%= id%>&fn=<%=fname%>&email=<%= email%>&add=<%= address%>&tp=<%= tp%>&pm=<%= p_method%>&cn=<%= c_number%>&cvc=<%= cvc%>&exp=<%= exp_date%>">Update</a></td>
